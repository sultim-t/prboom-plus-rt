...
boolean R_SetupPortalClipsegs(short *top, short *bottom)
{
   ...
   //first thing's first, extend the first solidseg to one column left of the first open post
   while(i < viewwidth && top[i] + 1 >= bottom[i]) i++;

   // first open post found, so set the last closed post to the last closed post (i - 1);
