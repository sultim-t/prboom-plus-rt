...
//-----------------------------------------------------------------------------
//
// $Id: Am_map.c,v 1.1 2000/04/09 18:15:01 proff_fs Exp $
//
...
//
// Sets the marknums[i] variables to the lumpnum for each digit
// Passed nothing, returns nothing;
//
static void AM_loadPics(void)
{
  ...
  for (i=0;i<10;i++)
  {
    ...
    marknums[i] = W_GetNumForName(namebuf);
  }
}
...
void AM_Coordinates(const mobj_t *mo, fixed_t *x, fixed_t *y, fixed_t *z, int *sectornum)
{
  *z = followplayer || !map_point_coordinates ? *x = mo->x, *y = mo->y, mo->z :
    R_PointInSubsector(*x = m_x+m_w/2, *y = m_y+m_h/2)->sector->floorheight;
  *sectornum = R_PointInSubsector(m_x+m_w/2, m_y+m_h/2)->sector->iSectorID;
}
...
{
  V_FillBlock(0,0,0,f_w,f_h,color);
}
...
{
#ifdef GL_DOOM
  // proff 11/99: using the OpenGL line routine
    ...
    if (markpoints[i].x != -1)
    {
      ...
      do
      {
        ...
        if (fx >= f_x && fx < f_w - w && fy >= f_y && fy < f_h - h)
          V_DrawPatchFromNum(fx, fy, FB, marknums[d]);
...
//
// $Log: Am_map.c,v $
// Revision 1.1  2000/04/09 18:15:01  proff_fs
// Initial revision
//
