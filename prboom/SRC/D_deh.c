...
//-----------------------------------------------------------------------------
//
// $Id: D_deh.c,v 1.1 2000/04/09 18:04:56 proff_fs Exp $
//
...
static const char
rcsid[] = "$Id: D_deh.c,v 1.1 2000/04/09 18:04:56 proff_fs Exp $";
...
H_boolean deh_pars = FALSE; // in wi_stuff to allow pars in modified games
...
H_boolean deh_GetData(char *, char *, longlong *, char **, FILE *);
H_boolean deh_procStringSub(char *, char *, char *, FILE *);
...
static H_boolean includenotext = false;  
...
void ProcessDehFile(char *filename, char *outfilename, int lumpnum)
{
  ...
  if (outfilename && *outfilename && !fileout)
    {
      static H_boolean firstfile = true; // to allow append to output log
      if (!strcmp(outfilename, "-"))
        fileout = stdout;
      else
        if (!(fileout=fopen(outfilename, firstfile ? "wt" : "at")))
          {
            lprintf(LO_ERROR, "Could not open -dehout file %s\n... using stdout.\n",
                   outfilename);
            fileout = stdout;
          }
      firstfile = false;
      ...
      if (!strnicmp(inbuffer,"INCLUDE",7)) // include a file
        {
          ...
          char *nextfile;
          H_boolean oldnotext = includenotext;       // killough 10/98
...
static void deh_procBexCodePointers(DEHFILE *fpin, FILE* fpout, char *line)
{
  ...
  int i; // looper
  H_boolean found; // know if we found this one during lookup or not
...
static void deh_procThing(DEHFILE *fpin, FILE* fpout, char *line)
{
  char key[DEH_MAXKEYLEN];
  char inbuffer[DEH_BUFFERMAX];
  longlong value;      // All deh values are ints or longs
  int indexnum;
  int ix;
  ...
  while (!feof(fpin) && *inbuffer && (*inbuffer != ' '))
    {
      ...
      if (!strcasecmp(key,deh_weapon[0]))  // Ammo type
        weaponinfo[indexnum].ammo = (int)value;
      else
...
static void deh_procText(DEHFILE *fpin, FILE* fpout, char *line)
{
  ...
  H_boolean found = FALSE;  // to allow early exit once found
...
static void deh_procStrings(DEHFILE *fpin, FILE* fpout, char *line)
{
  ...
  H_boolean found = false;  // looking for string continuation
...
// Returns: H_boolean: True if string found, false if not
//
static H_boolean deh_procStringSub(char *key, char *lookfor, char *newstring, FILE *fpout)
{
  H_boolean found; // loop exit flag
  int i;  // looper
...
static H_boolean deh_GetData(char *s, char *k, longlong *l, char **strval, FILE *fpout)
{
  char *t;  // current char
  long val; // to hold value of pair
  char buffer[DEH_MAXKEYLEN];  // to hold key in progress
  H_boolean okrc = TRUE;  // assume good unless we have problems
  int i;  // iterator
...
//
// $Log: D_deh.c,v $
// Revision 1.1  2000/04/09 18:04:56  proff_fs
// Initial revision
//
