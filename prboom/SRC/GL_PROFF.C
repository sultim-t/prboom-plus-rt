...
void gld_AddPatchToTexture(GLTexture *gltexture, unsigned char *buffer, patch_t *patch, int originx, int originy)
{
  int x,y,j;
...
void gld_AddPatchToTexture_CharTrans(GLTexture *gltexture, unsigned char *buffer, patch_t *patch, int originx, int originy, unsigned char cm)
{
  int x,y,j;
  column_t *p_bColumn_t;
  byte *p_bColumn;
...
void gld_AddPatchToTexture_SpriteTrans(GLTexture *gltexture, unsigned char *buffer, patch_t *patch, int originx, int originy, unsigned char cm)
{
  int x,y,j;
  column_t *p_bColumn_t;
  byte *p_bColumn;
...
void gld_AddFlatToTexture(GLTexture *gltexture, unsigned char *buffer, unsigned char *flat)
{
  int x,y,pos;
  unsigned char *playpal=W_CacheLumpName("PLAYPAL", PU_STATIC);
...
void gld_DrawPatch(int x, int y, int lump, unsigned char cm, H_boolean flipped, H_boolean stretched)
{ 
  GLTexture *gltexture;
...
void gld_DrawPatchFromMem(int x, int y, patch_t *patch, unsigned char cm, H_boolean flipped, H_boolean stretched)
{ 
  GLTexture *gltexture;
  int size;
  unsigned char *buffer;
...
void gld_AddSpriteToRenderQueue(mobj_t *pSpr,int lump, H_boolean flip)
{
...
static void gld_DrawSeg(drawseg_t *drawseg, H_boolean sky_only)
{
  walldef_t walldef;
  seg_t *seg;
  line_t *line;
...
static void gld_DrawFlat(int num, H_boolean ceiling)
{
...
static H_boolean *sectorclosed=NULL; // list of all sectors with true if sector closed
static H_boolean *sectorrendered=NULL; // true if sector rendered (only here for malloc)
...
static void gld_PrecalculateSector(int num)
{
  ...
  currentsector=num;
  lineadded=GLRealloc(lineadded,sectors[num].linecount*sizeof(H_boolean));
  if (!lineadded)
  {
...
void gld_PreprocessLevel(void)
{
  ...
  sectorclosed=GLRealloc(sectorclosed,numsectors*sizeof(H_boolean));
  sectorrendered=GLRealloc(sectorrendered,numsectors*sizeof(H_boolean));
