...
//-----------------------------------------------------------------------------
//
// $Id: Hu_lib.c,v 1.1 2000/04/09 18:19:47 proff_fs Exp $
//
...
static const char
rcsid[] = "$Id: Hu_lib.c,v 1.1 2000/04/09 18:19:47 proff_fs Exp $";
...
// H_boolean : whether the screen is always erased
#define noterased viewwindowx

extern H_boolean  automapactive;  // in AM_map.c
...
// Returns nothing
//
void HUlib_initTextLine(hu_textline_t* t, int x, int y, patchnum_t *f, int sc,
...
void HUlib_drawTextLine(hu_textline_t* l, H_boolean drawcursor)
{
  ...
  for (i=0;i<l->len;i++)
  {
    ...
    else  if (c != ' ' && c >= l->sc && c <= 127)
    {
      ...
      if (x+w > 320)
        break;
      // killough 1/18/98 -- support multiple lines:
      V_DrawPatchTransStretchedFromNum(x, y, FG, l->f[c - l->sc].lumpnum, l->cm);
      x += w;
    }
    else
    {
  ...
  if (drawcursor && x + SHORT(l->f['_' - l->sc].width) <= 320)
  {
    // killough 1/18/98 -- support multiple lines
    V_DrawPatchStretchedFromNum(x, y, FG, l->f['_' - l->sc].lumpnum);
  }
}
...
void HUlib_eraseTextLine(hu_textline_t* l)
{
  ...
  if (!automapactive && viewwindowx && l->needsupdate)
  {
    ...
    for (y=l->y,yoffset=y*SCREENWIDTH ; y<((l->y+lh)*SCREENHEIGHT/200) ; y++,yoffset+=SCREENWIDTH)
      {
...
void HUlib_initSText(hu_stext_t* s, int x, int y, int h, patchnum_t* font,
                     int startchar, int cm, H_boolean* on)
{
...
// Returns nothing
//
void HUlib_initMText(hu_mtext_t *m, int x, int y, int w, int h, patchnum_t* font,
                     int startchar, int cm, patchnum_t* bgfont, H_boolean *on)
{
  int i;
  ...
  for (i=0; i<hud_msg_lines; i++, y -= HU_REFRESHSPACING)
    HUlib_initTextLine(&m->l[i], x, y, font, startchar, cm);
}
...
// Returns nothing
//
void HUlib_drawMBg(int x, int y, int w, int h, patchnum_t *bgp)
{
  int xs = bgp[0].width;
  int ys = bgp[0].height;
  int i,j;
  ...
  // top rows
  V_DrawPatchStretchedFromNum(x, y, FG, bgp[0].lumpnum);    // ul
  for (j=x+xs;j<x+w-xs;j+=xs)           // uc
    V_DrawPatchStretchedFromNum(j, y, FG, bgp[1].lumpnum);
  V_DrawPatchStretchedFromNum(j, y, FG, bgp[2].lumpnum);    // ur

  // middle rows
  for (i=y+ys;i<y+h-ys;i+=ys)
  {
    V_DrawPatchStretchedFromNum(x, i, FG, bgp[3].lumpnum);    // cl
    for (j=x+xs;j<x+w-xs;j+=xs)           // cc
      V_DrawPatchStretchedFromNum(j, i, FG, bgp[4].lumpnum);  
    V_DrawPatchStretchedFromNum(j, i, FG, bgp[5].lumpnum);    // cr
  }

  // bottom row
  V_DrawPatchStretchedFromNum(x, i, FG, bgp[6].lumpnum);    // ll
  for (j=x+xs;j<x+w-xs;j+=xs)           // lc
    V_DrawPatchStretchedFromNum(j, i, FG, bgp[7].lumpnum);  
  V_DrawPatchStretchedFromNum(j, i, FG, bgp[8].lumpnum);    // lr
}
...
void HUlib_drawMText(hu_mtext_t* m)
{
  ...
  for (i=0 ; i<m->nl ; i++)
	...
	idx += m->nl; // handle queue of lines
      ...
      m->l[idx].y = m->y+i*HU_REFRESHSPACING;
...
static void HUlib_eraseMBg(hu_mtext_t* m)
{
  ...
  if (!automapactive && viewwindowx)
  {
    ...
    for (y=m->y,yoffset=y*SCREENWIDTH ; y<((m->y+lh*(hud_msg_lines+2))*SCREENHEIGHT/200) ; y++,yoffset+=SCREENWIDTH)
    {
      if (y < viewwindowy || y >= viewwindowy + viewheight)
        R_VideoErase(yoffset, SCREENWIDTH); // erase entire line
      else
...
void HUlib_initIText(hu_itext_t *it, int x, int y, patchnum_t *font,
                     int startchar, int cm, H_boolean *on)
{
...
//
H_boolean HUlib_keyInIText(hu_itext_t *it, unsigned char ch)
{
...
//
// $Log: Hu_lib.c,v $
// Revision 1.1  2000/04/09 18:19:47  proff_fs
// Initial revision
//
