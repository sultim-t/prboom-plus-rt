...
extern H_boolean  message_dontfuckwithme;
          
extern H_boolean chat_on;          // in heads-up code
...
H_boolean messageNeedsInput; // timed message = no input from user     
...
H_boolean inhelpscreens; // indicates we are in or just left a help screen

H_boolean menuactive;    // The menus are up
...
void M_StartMessage(char *string,void *routine,H_boolean input);
...
void M_DrawMainMenu(void)
{
  V_DrawPatchStretchedFromName(94,2,0,"M_DOOM");
}
...
void M_DrawEpisode(void)
{
  V_DrawPatchStretchedFromName(54,38,0,"M_EPISOD");
}
...
void M_DrawNewGame(void)
{
  V_DrawPatchStretchedFromName(96,14,0,"M_NEWG");
  V_DrawPatchStretchedFromName(54,38,0,"M_SKILL");
}
...
void M_DrawLoad(void)
{
  ...
  V_DrawPatchStretchedFromName(72,LOADGRAPHIC_Y,0,"M_LOADG");
...
void M_DrawSaveLoadBorder(int x,int y)
{
  ...
  V_DrawPatchStretchedFromName(x-8,y+7,0,"M_LSLEFT");
  
  for (i = 0 ; i < 24 ; i++)
    {
      V_DrawPatchStretchedFromName(x,y+7,0,"M_LSCNTR");
      x += 8;
    }

  V_DrawPatchStretchedFromName(x,y+7,0,"M_LSRGHT");
}
...
void M_DrawSave(void)
{
  ...
  //jff 3/15/98 use symbolic load position
  V_DrawPatchStretchedFromName(72,LOADGRAPHIC_Y,0,"M_SAVEG");
  for (i = 0 ; i < load_end ; i++)
    {
      M_DrawSaveLoadBorder(LoadDef.x,LoadDef.y+LINEHEIGHT*i);
      M_WriteText(LoadDef.x,LoadDef.y+LINEHEIGHT*i,savegamestrings[i]);
...
void M_DrawOptions(void)
{
  V_DrawPatchStretchedFromName(108,15,0,"M_OPTTTL");

  V_DrawPatchStretchedFromName(OptionsDef.x + 120,OptionsDef.y+LINEHEIGHT*messages,0,msgNames[showMessages]);
...
void M_DrawSound(void)
{
  V_DrawPatchStretchedFromName(60,38,0,"M_SVOL");
...
void M_DrawMouse(void)
{
  ...
  V_DrawPatchStretchedFromName(60,38,0,"M_MSENS");
...
H_boolean setup_active      = false; // in one of the setup screens
H_boolean set_keybnd_active = false; // in key binding setup screens
H_boolean set_weapon_active = false; // in weapons setup screen
H_boolean set_status_active = false; // in status bar/hud setup screen
H_boolean set_auto_active   = false; // in automap setup screen
H_boolean set_enemy_active  = false; // in enemies setup screen
H_boolean set_mess_active   = false; // in messages setup screen
H_boolean set_chat_active   = false; // in chat string setup screen
H_boolean setup_select      = false; // changing an item
H_boolean setup_gather      = false; // gathering keys for value
H_boolean colorbox_active   = false; // color palette being shown
H_boolean default_verify    = false; // verify reset defaults decision
H_boolean set_general_active = false;
H_boolean set_compat_active = false;
...
void M_DrawBackgr(char* patchname, byte* back_dest)
...
{
  V_DrawPatchStretchedFromName(124,15,0,"M_SETUP");
}
...
void M_DrawItem(setup_menu_t* s)
{
  ...
  if (flags & S_RESET)
    ...
    V_DrawPatchStretchedFromName(x,y,0,ResetButtonName[flags & (S_HILITE|S_SELECT) ? whichSkull : 0]);
...
void M_DrawSetting(setup_menu_t* s)
{
  ...
  if (flags & S_COLOR) // Automap paint chip
    {
      ...
      // proff 12/6/98: Drawing of colorchips completly changed for hi-res, it now uses a patch
      // draw the paint chip
      V_DrawPatchStretchedFromMem(x,y-1,0,M_MakeColChip((byte)ch,PAL_BLACK));
       
      if (!ch) // don't show this item in automap mode
	      V_DrawPatchStretchedFromName(x+1,y,0,"M_PALNO");
      return;
    }
  ...
  if (flags & S_STRING)
    ...
    V_FillBlock(((x+cursor_start-1)*SCREENWIDTH)/320, (y*SCREENHEIGHT)/200, 0,
...
void M_DrawDefVerify()
{
  ...
  V_DrawPatchStretchedFromName(VERIFYBOXXORG,VERIFYBOXYORG,0,"M_VBOX");
...

{
  ...
  V_DrawPatchStretchedFromName(84,2,0,"M_KEYBND");
  M_DrawInstructions();
...
void M_DrawWeapons(void)
{
  ...
  M_DrawBackground("FLOOR4_6", screens[0]); // Draw background
  V_DrawPatchStretchedFromName(109,2,0,"M_WEAP");
  M_DrawInstructions();
  M_DrawScreenItems(current_setup_menu);
...

{
  ...
  M_DrawBackground("FLOOR4_6", screens[0]); // Draw background
  V_DrawPatchStretchedFromName(59,2,0,"M_STAT");
  M_DrawInstructions();
  M_DrawScreenItems(current_setup_menu);
...
void M_DrawColPal()
{
  ...
  V_DrawPatchStretchedFromName(COLORPALXORIG-5,COLORPALYORIG-5,0,"M_COLORS");
  ...
  // proff 12/6/98: Drawing of colorchips completly changed for hi-res, it now uses a patch
  V_DrawPatchStretchedFromMem(cpx,cpy,0,(patch_t *)curchip); // PROFF_GL_FIX
}
...

{
  ...
  M_DrawBackground("FLOOR4_6", screens[0]); // Draw background
  V_DrawPatchStretchedFromName(109,2,0,"M_AUTO");
  M_DrawInstructions();
  M_DrawScreenItems(current_setup_menu);
...

{
  ...
  M_DrawBackground("FLOOR4_6", screens[0]); // Draw background
  V_DrawPatchStretchedFromName(114,2,0,"M_ENEM");
  M_DrawInstructions();
  M_DrawScreenItems(current_setup_menu);
...
void M_DrawGeneral(void)
{
  ...
  M_DrawBackground("FLOOR4_6", screens[0]); // Draw background
  V_DrawPatchStretchedFromName(114,2,0,"M_GENERL");
  M_DrawInstructions();
  M_DrawScreenItems(current_setup_menu);
...
void M_DrawCompat(void)
{
  ...
  M_DrawBackground("FLOOR4_6", screens[0]); // Draw background
  V_DrawPatchStretchedFromName(52,2,0,"M_COMPAT");
  M_DrawInstructions();
  M_DrawScreenItems(current_setup_menu);
...

{
  inhelpscreens = true;
  M_DrawBackground("FLOOR4_6", screens[0]); // Draw background
  V_DrawPatchStretchedFromName(103,2,0,"M_MESS");
  M_DrawInstructions();
  M_DrawScreenItems(current_setup_menu);
  if (default_verify)
    M_DrawDefVerify();
...

{
  inhelpscreens = true;
  M_DrawBackground("FLOOR4_6", screens[0]); // Draw background
  V_DrawPatchStretchedFromName(83,2,0,"M_CHAT");
  M_DrawInstructions();
  M_DrawScreenItems(current_setup_menu);
...
static H_boolean shiftdown = false; // phares 4/10/98: SHIFT key down or not
...
void M_DrawExtHelp(void)
{
  ...
  V_DrawPatchStretchedFromName(0,0,0,namebfr);
}
...
void M_DrawMenuString(int cx, int cy, int color)
{
  ...
  while (*ch)
	...
	break;
      ...
      V_DrawPatchTransStretchedFromNum(cx,cy,0,hu_font[c].lumpnum,color);
...
void M_DrawCredits(void)     // killough 10/98: credit screen
{
  ...
  V_DrawPatchTransStretchedFromName(115,9,0, "PRBOOM",CR_GOLD);
...
H_boolean M_Responder (event_t* ev)
{
  int    ch;
  int    i;
...
// phares 4/4/98:
    ...
    }                                 // here using the 'shiftdown' H_boolean.
	...
	for (i=0;i<max;i++)
	  {
	    if (currentMenu->menuitems[i].name[0])
	      V_DrawPatchStretchedFromName(x,y,0,currentMenu->menuitems[i].name);
	    y += LINEHEIGHT;
	  }
	
	// DRAW SKULL

	V_DrawPatchStretchedFromName(x + SKULLXOFF,currentMenu->y - 5 + itemOn*LINEHEIGHT,0,skullName[whichSkull]);
      }
}
...
void M_DrawThermo(int x,int y,int thermWidth,int thermDot )
{
  ...
  xx = x;
  V_DrawPatchStretchedFromName(xx,y,0,"M_THERML");
  xx += 8;
  for (i=0;i<thermWidth;i++)
    {
      V_DrawPatchStretchedFromName(xx,y,0,"M_THERMM");
      xx += 8;
    }
  V_DrawPatchStretchedFromName(xx,y,0,"M_THERMR");

  V_DrawPatchStretchedFromName((x+8) + thermDot*8,y,0,"M_THERMO");
}
...
void M_DrawEmptyCell (menu_t* menu,int item)
{
  V_DrawPatchStretchedFromName(menu->x - 10,menu->y+item*LINEHEIGHT - 1, 0, "M_CELL1");
}
...
void M_DrawSelCell (menu_t* menu,int item)
{
  V_DrawPatchStretchedFromName(menu->x - 10,menu->y+item*LINEHEIGHT - 1, 0, "M_CELL2");
}
