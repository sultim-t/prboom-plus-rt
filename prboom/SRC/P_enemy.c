...
//-----------------------------------------------------------------------------
//
// $Id: P_enemy.c,v 1.1 2000/04/09 18:16:20 proff_fs Exp $
//
...
static const char
rcsid[] = "$Id: P_enemy.c,v 1.1 2000/04/09 18:16:20 proff_fs Exp $";
...
static H_boolean P_CheckMeleeRange(mobj_t *actor)
{
...
static H_boolean P_HitFriend(mobj_t *actor)
{
...
// P_CheckMissileRange
//
static H_boolean P_CheckMissileRange(mobj_t *actor)
{
  fixed_t dist;
...
static H_boolean P_IsOnLift(const mobj_t *actor)
{
...
static H_boolean P_Move(mobj_t *actor, H_boolean dropoff) // killough 9/12/98
{
...
static H_boolean P_SmartMove(mobj_t *actor)
{
...
static H_boolean P_TryWalk(mobj_t *actor)
{
...
static H_boolean PIT_AvoidDropoff(line_t *line)
{
...
static H_boolean P_IsVisible(mobj_t *actor, mobj_t *mo, H_boolean allaround)
{
  if (!allaround)
    {
...
static H_boolean PIT_FindTarget(mobj_t *mo)
{
...
static H_boolean P_LookForPlayers(mobj_t *actor, H_boolean allaround)
{
  player_t *player;
  int stop, stopc, c;
...
static H_boolean P_LookForMonsters(mobj_t *actor, H_boolean allaround)
{
  thinker_t *cap, *th;
...
static H_boolean P_LookForTargets(mobj_t *actor, int allaround)
{
...
static H_boolean P_HelpFriend(mobj_t *actor)
{
  thinker_t *cap, *th;
...
H_boolean PIT_VileCheck(mobj_t *thing)
{
  int     maxdist;
  H_boolean check;
...
{
  mo->angle += ((ulonglong) mo->state->misc1 << 32) / 360;
}
...
//
// $Log: P_enemy.c,v $
// Revision 1.1  2000/04/09 18:16:20  proff_fs
// Initial revision
//
