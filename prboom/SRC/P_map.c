...
//-----------------------------------------------------------------------------
//
// $Id: P_map.c,v 1.1 2000/04/09 18:16:46 proff_fs Exp $
//
...
static const char
rcsid[] = "$Id: P_map.c,v 1.1 2000/04/09 18:16:46 proff_fs Exp $";
...
static H_boolean telefrag;   // killough 8/9/98: whether to telefrag at exit

static H_boolean PIT_StompThing (mobj_t *thing)
{
  fixed_t blockdist;
...
// killough 8/9/98
H_boolean P_TeleportMove(mobj_t *thing, fixed_t x, fixed_t y, H_boolean boss)
{
...
static H_boolean PIT_CrossLine(line_t *ld)
{
  return 
...
static H_boolean PIT_CheckLine(line_t *ld) // killough 3/26/98: make static
{
...
static H_boolean PIT_CheckThing(mobj_t *thing) // killough 3/26/98: make static
{
  fixed_t blockdist;
  int damage;
  ...
  if (thing->flags & MF_SPECIAL)
    {
      longlong solid = thing->flags & MF_SOLID;
...
H_boolean Check_Sides(mobj_t *actor, int x, int y)
{
...
H_boolean P_CheckPosition(mobj_t *thing, fixed_t x, fixed_t y) 
{
  int xl, xh, yl, yh, bx, by;
  subsector_t *newsubsec;
...
H_boolean P_TryMove(mobj_t *thing, fixed_t x, fixed_t y, H_boolean dropoff)
{
  fixed_t oldx, oldy;
...
static H_boolean PIT_ApplyTorque(line_t *ld)
{
...
static H_boolean P_ThingHeightClip(mobj_t *thing)
{
  H_boolean onfloor = thing->z == thing->floorz;
...
static void P_HitSlideLine(line_t *ld)
{
  ...
  fixed_t movelen;
  fixed_t newlen;
  H_boolean icyfloor;  // is floor icy?
...
static H_boolean PTR_SlideTraverse(intercept_t *in)
{
  line_t *li;
...
static longlong aim_flags_mask; // killough 8/2/98: for more intelligent autoaiming
...
//
static H_boolean PTR_AimTraverse (intercept_t *in)
{
...
// PTR_ShootTraverse
//
static H_boolean PTR_ShootTraverse(intercept_t *in)
{
...
fixed_t P_AimLineAttack(mobj_t *t1,angle_t angle,fixed_t distance, longlong mask)
{
  fixed_t x2, y2;
...
static H_boolean PTR_UseTraverse(intercept_t *in)
{
...
static H_boolean PTR_NoWayTraverse(intercept_t *in)
{
...
static H_boolean PIT_RadiusAttack(mobj_t *thing)
{
  fixed_t dx, dy, dist;
...
static H_boolean crushchange, nofit;

//
// PIT_ChangeSector
//

static H_boolean PIT_ChangeSector(mobj_t *thing)
{
  mobj_t *mo;
...
// P_ChangeSector
//
static H_boolean P_ChangeSector(sector_t *sector,H_boolean crunch)
{
  int x, y;
...
H_boolean P_CheckSector(sector_t *sector,H_boolean crunch)
{
  msecnode_t *n;
...
static H_boolean PIT_GetSectors(line_t *ld)
{
  if (tmbbox[BOXRIGHT]  <= ld->bbox[BOXLEFT]   ||
      tmbbox[BOXLEFT]   >= ld->bbox[BOXRIGHT]  ||
      tmbbox[BOXTOP]    <= ld->bbox[BOXBOTTOM] ||
...
//
// $Log: P_map.c,v $
// Revision 1.1  2000/04/09 18:16:46  proff_fs
// Initial revision
//
