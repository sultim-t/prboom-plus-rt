...
//-----------------------------------------------------------------------------
//
// $Id: P_maputl.c,v 1.1 2000/04/09 18:01:10 proff_fs Exp $
//
...
static const char
rcsid[] = "$Id: P_maputl.c,v 1.1 2000/04/09 18:01:10 proff_fs Exp $";
...
H_boolean ThingIsOnLine(mobj_t *t, line_t *l)
{
...
H_boolean P_BlockLinesIterator(int x, int y, H_boolean func(line_t*))
{
  int        offset;
...
H_boolean P_BlockThingsIterator(int x, int y, H_boolean func(mobj_t*))
{
  mobj_t *mobj;
  if (!(x<0 || y<0 || x>=bmapwidth || y>=bmapheight))
...
H_boolean PIT_AddLineIntercepts(line_t *ld)
{
  int       s1;
  int       s2;
  fixed_t   frac;
...
H_boolean PIT_AddThingIntercepts(mobj_t *thing)
{
  fixed_t   x1, y1;
  fixed_t   x2, y2;
  int       s1, s2;
...
H_boolean P_TraverseIntercepts(traverser_t func, fixed_t maxfrac)
{
  intercept_t *in = NULL;
...
H_boolean P_PathTraverse(fixed_t x1, fixed_t y1, fixed_t x2, fixed_t y2,
                       int flags, H_boolean trav(intercept_t *))
{
  fixed_t xt1, yt1;
  fixed_t xt2, yt2;
  fixed_t xstep, ystep;
...
//
// $Log: P_maputl.c,v $
// Revision 1.1  2000/04/09 18:01:10  proff_fs
// Initial revision
//
