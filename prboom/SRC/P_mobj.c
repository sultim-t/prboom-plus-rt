...
//-----------------------------------------------------------------------------
//
// $Id: P_mobj.c,v 1.1 2000/04/09 18:20:37 proff_fs Exp $
//
...
static const char
rcsid[] = "$Id: P_mobj.c,v 1.1 2000/04/09 18:20:37 proff_fs Exp $";
...
H_boolean P_SetMobjState(mobj_t* mobj,statenum_t state)
{
  ...
  H_boolean ret = true;                         // return value
...
void P_RespawnSpecials (void)
{
  ...
  // spawn it
  z = ((mobjinfo[i].flags & MF_SPAWNCEILING)>0) ? ONCEILINGZ : ONFLOORZ;
...
void P_SpawnMapThing (mapthing_t* mthing)
{
  ...
  if (deathmatch && ((mobjinfo[i].flags & MF_NOTDMATCH)>0))
    return;

  // don't spawn any monsters if -nomonsters

  if (nomonsters && (i == MT_SKULL || ((mobjinfo[i].flags & MF_COUNTKILL)>0)))
    return;
  ...
  // killough 7/19/98: autoaiming was not in original beta
    {
      ...
      longlong mask = demo_version < 203 ? 0 : MF_FRIEND;
      do
	{
...
//
// $Log: P_mobj.c,v $
// Revision 1.1  2000/04/09 18:20:37  proff_fs
// Initial revision
//
