...
//-----------------------------------------------------------------------------
//
// $Id: P_spec.c,v 1.1 2000/04/09 18:17:27 proff_fs Exp $
//
...
static const char
rcsid[] = "$Id: P_spec.c,v 1.1 2000/04/09 18:17:27 proff_fs Exp $";
...
H_boolean P_CanUnlockGenDoor(line_t *line, player_t *player)
{
...
H_boolean P_IsSecret(sector_t *sec)
{
...
H_boolean P_WasSecret(sector_t *sec)
{
...
H_boolean         levelTimer;
int             levelTimeCount;
H_boolean         levelFragLimit;      // Ty 03/18/98 Added -frags support
...
static void Add_WallScroller(longlong dx, longlong dy, const line_t *l,
                             int control, int accel)
{
  ...
  y = (int)((dy * l->dx - dx * l->dy) / d);   // Use longlong arithmetic
...
H_boolean PIT_PushThing(mobj_t* thing)
{
      ...
      if (speed > 0 && demo_version >= 203)
        {
          ...
          speed = (int)(((longlong) tmpusher->magnitude << 23) / (x*x+y*y+1));
        }
...
//
// $Log: P_spec.c,v $
// Revision 1.1  2000/04/09 18:17:27  proff_fs
// Initial revision
//
