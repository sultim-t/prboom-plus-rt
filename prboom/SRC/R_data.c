...
//-----------------------------------------------------------------------------
//
// $Id: R_data.c,v 1.1 2000/04/09 18:13:34 proff_fs Exp $
//
...
static const char
rcsid[] = "$Id: R_data.c,v 1.1 2000/04/09 18:13:34 proff_fs Exp $";
...
// proff 11/19/98: Reworked for 2 new filters
//

#define TSC 12        // number of fixed point digits in filter percent

static void R_GenerateTranMap(int progress, byte *playpal, byte *tmap, int filter_pct)
{
...
static void R_CheckTranMap(int progress, char *lumpname, char *filename, int filter_pct, byte *tmap)
{
  int lump = W_CheckNumForName(lumpname);
  byte *cache_tranmap = NULL;

  // If a tranlucency filter map lump is present, use it

  if (lump != -1)  // Set a pointer to the translucency filter maps.
  {
    cache_tranmap = W_CacheLumpNum(lump, PU_STATIC);
    memcpy(tmap, cache_tranmap, 256*256);
    Z_ChangeTag(cache_tranmap, PU_CACHE);
  ...
  else
  {
    ...
    FILE *cachefp;
    strcpy(fname, D_DoomExeDir());
    strcat(fname, filename);
    cachefp = fopen(fname,"r+b");

    if (!cachefp ? cachefp = fopen(fname,"wb") , 1 :
        fread(&cache, 1, sizeof cache, cachefp) != sizeof cache ||
        cache.pct != filter_pct ||
        memcmp(cache.playpal, playpal, sizeof cache.playpal) ||
        fread(tmap, 256, 256, cachefp) != 256 )
    {
#ifndef GL_DOOM
      R_GenerateTranMap(progress,playpal,tmap,filter_pct);
#endif
...
int tran_filter_pct25 = 25;     // filter percent for filter 1 (25%)
int tran_filter_pct   = 66;     // filter percent
int tran_filter_pct75 = 75;     // filter percent for filter 3 (75%)

void R_InitTranMap(int progress)
{
  main_tranmap = Z_Malloc(256*256*3, PU_STATIC, 0);

  R_CheckTranMap( 0,"TRAN25","/tran25.dat",
                  tran_filter_pct25,main_tranmap+TRANMAP_POS_1);
  R_CheckTranMap( progress,"TRANMAP","/tranmap.dat",
                  tran_filter_pct,main_tranmap+TRANMAP_POS_2);
  R_CheckTranMap( 0,"TRAN75","/tran75.dat",
                  tran_filter_pct75,main_tranmap+TRANMAP_POS_3);
}
...
//
// $Log: R_data.c,v $
// Revision 1.1  2000/04/09 18:13:34  proff_fs
// Initial revision
//
