...
//-----------------------------------------------------------------------------
//
// $Id: St_stuff.c,v 1.1 2000/04/09 18:12:58 proff_fs Exp $
//
...
static const char
rcsid[] = "$Id: St_stuff.c,v 1.1 2000/04/09 18:12:58 proff_fs Exp $";
...
static H_boolean st_statusbaron;

// whether status bar chat is active
static H_boolean st_chat;

// value of st_chat before message popped up
static H_boolean st_oldchat;

// whether chat window has the cursor on
static H_boolean st_cursoron;

// !deathmatch
static H_boolean st_notdeathmatch;

// !deathmatch && st_statusbaron
static H_boolean st_armson;

// !deathmatch
static H_boolean st_fragson;
...
// used for evil grin
static H_boolean  oldweaponsowned[NUMWEAPONS];
...
void ST_refreshBackground(void)
{
  ...
  if (st_statusbaron)
    {
#ifdef GL_DOOM
      // proff 11/99: draw to the frontbuffer in OpenGL
      y=ST_Y;
      screen=1;
#endif
      V_DrawPatchStretchedFromName(ST_X, y, screen, "STBAR");
      ...
      if (netgame)
      {
        ...
        sprintf(namebuf, "STFB%d", displayplayer);
        V_DrawPatchStretchedFromName(ST_FX, y, screen, namebuf);
      }

      V_CopyRect(ST_X, y, screen, ST_SCALED_WIDTH, ST_SCALED_HEIGHT, ST_X, ST_SCALED_Y, FG);
    }
}
...
//  intercept cheats.
H_boolean ST_Responder(event_t *ev)
{
...
void ST_drawWidgets(H_boolean refresh)
{
  int i;
...
void ST_Drawer(H_boolean fullscreen, H_boolean refresh)
{
...
#ifdef GL_DOOM
  // proff 11/99: always draw everything in OpenGL, because there is no backbuffer
  if (st_statusbaron)
    ST_doRefresh();
#else
  if (st_firsttime)
...
static H_boolean st_stopped = true;
...
void ST_Init(void)
{
  ...
  ST_loadData();
// proff 08/18/98: Changed for high-res
  screens[4] = Z_Malloc(SCREENWIDTH*((ST_HEIGHT*SCREENHEIGHT/200)+1), PU_STATIC, 0);
...
//
// $Log: St_stuff.c,v $
// Revision 1.1  2000/04/09 18:12:58  proff_fs
// Initial revision
//
