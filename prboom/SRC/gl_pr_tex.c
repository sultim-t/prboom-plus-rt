...
extern void gld_AddPatchToTexture(GLTexture *gltexture, unsigned char *buffer, patch_t *patch, int originx, int originy);
...
GLTexture *gld_RegisterTexture(int texture_num)
{
  ...
  {
    patch=W_CacheLumpNum(texture->patches[i].patch,PU_STATIC);
