...
uint16 snd_buffer[65535];

void *I_UpdateSound(int len)
{
  ...
  leftout = (signed short *)snd_buffer;
  rightout = ((signed short *)snd_buffer)+1;
  step = 2;
...
{ 
  ...
  snd_stream_init(I_UpdateSound);
  thd_create(snd_thread, NULL);
  snd_stream_start(snd_samplerate,1);
