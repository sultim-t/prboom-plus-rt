/* $Id: gl_mipmap.c,v 1.1 2002/11/16 11:02:04 proff_fs Exp $ */
	      ...
	      GLenum typeout, void *dataout)
{
   ...
   p_glGetIntegerv(GL_UNPACK_ROW_LENGTH, &unpackrowlength);
   p_glGetIntegerv(GL_UNPACK_ALIGNMENT, &unpackalignment);
   p_glGetIntegerv(GL_UNPACK_SKIP_ROWS, &unpackskiprows);
   p_glGetIntegerv(GL_UNPACK_SKIP_PIXELS, &unpackskippixels);
   p_glGetIntegerv(GL_PACK_ROW_LENGTH, &packrowlength);
   p_glGetIntegerv(GL_PACK_ALIGNMENT, &packalignment);
   p_glGetIntegerv(GL_PACK_SKIP_ROWS, &packskiprows);
   p_glGetIntegerv(GL_PACK_SKIP_PIXELS, &packskippixels);
 ...
 */
GLint GLAPIENTRY
p_gluBuild1DMipmaps(GLenum target, GLint components,
		  GLsizei width, GLenum format, GLenum type, const void *data)
{
   ...
   p_glGetIntegerv(GL_MAX_TEXTURE_SIZE, &max_width);
   ...
   for (l = 0; l < levels; l++) {
      p_glTexImage1D(GL_TEXTURE_1D, l, components, new_width, 0,
...
GLint GLAPIENTRY
p_gluBuild2DMipmaps(GLenum target, GLint components,
		  ...
		  GLenum type, const void *data)
{
   ...
   p_glGetIntegerv(GL_MAX_TEXTURE_SIZE, &maxsize);
   ...
   if (w != width || h != height) {
	 ...
	 return GLU_OUT_OF_MEMORY;
      ...
      error = p_gluScaleImage(format, width, height, type, data,
			    w, h, type, image);
      if (error) {
	 retval = error;
	 done = GL_TRUE;
   ...
   while (!done) {
	 ...
	 p_glPixelStorei(GL_UNPACK_SKIP_PIXELS, 0);
      ...
      p_glTexImage2D(target, level, components, w, h, 0, format, type, image);
	 ...
	 return GLU_OUT_OF_MEMORY;
      ...
      error = p_gluScaleImage(format, w, h, type, image,
   ...
   p_glPixelStorei(GL_UNPACK_ROW_LENGTH, unpackrowlength);
   p_glPixelStorei(GL_UNPACK_ALIGNMENT, unpackalignment);
   p_glPixelStorei(GL_UNPACK_SKIP_ROWS, unpackskiprows);
   p_glPixelStorei(GL_UNPACK_SKIP_PIXELS, unpackskippixels);
   p_glPixelStorei(GL_PACK_ROW_LENGTH, packrowlength);
   p_glPixelStorei(GL_PACK_ALIGNMENT, packalignment);
   p_glPixelStorei(GL_PACK_SKIP_ROWS, packskiprows);
   p_glPixelStorei(GL_PACK_SKIP_PIXELS, packskippixels);
