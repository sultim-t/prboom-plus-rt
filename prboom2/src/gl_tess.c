...
void GLAPIENTRY
p_gluTessCallback(GLUtriangulatorObj * tobj, GLenum which,
		void (GLCALLBACK * fn) ())
{
   switch (which) {
   case GLU_BEGIN:
...
void GLAPIENTRY
p_gluDeleteTess(GLUtriangulatorObj * tobj)
{
...
void GLAPIENTRY
p_gluBeginPolygon(GLUtriangulatorObj * tobj)
{
/*
	if(tobj->error!=GLU_NO_ERROR)
		return;
...
void GLAPIENTRY
p_gluEndPolygon(GLUtriangulatorObj * tobj)
{
...
void GLAPIENTRY
p_gluNextContour(GLUtriangulatorObj * tobj, GLenum type)
{
   if (tobj->error != GLU_NO_ERROR)
      return;
   if (tobj->current_polygon == NULL) {
...
void GLAPIENTRY
p_gluTessVertex(GLUtriangulatorObj * tobj, GLdouble v[3], void *data)
{
