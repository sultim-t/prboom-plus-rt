...
static void cheat_noclip()
{
  ...
  doom_printf((plyr->cheats ^= CF_NOCLIP) & CF_NOCLIP ? 
...
static void cheat_friction()
{
  doom_printf(                      // Ty 03/27/98 - *not* externalized
...
static void cheat_hom()
{
  ...
  doom_printf((autodetect_hom = !autodetect_hom) ? "HOM Detection On" :
    "HOM Detection Off");
}
...
static void cheat_fast()
{
  doom_printf((fastparm = !fastparm) ? "Fast Monsters On" : 
...
char buf[1];
{
  ...
  if (*buf == 'b')  // Ty 03/27/98 - strings *not* externalized
    if ((plyr->backpack = !plyr->backpack))
      for (doom_printf("Backpack Added"),   a=0 ; a<NUMAMMO ; a++)
        plyr->maxammo[a] <<= 1;
    else
      for (doom_printf("Backpack Removed"), a=0 ; a<NUMAMMO ; a++)
        {
...
static void cheat_smart()
{
  ...
  doom_printf((monsters_remember = !monsters_remember) ? 
...
static void cheat_pitch()
{
  ...
  doom_printf((pitched_sounds = !pitched_sounds) ? "Pitch Effects Enabled" :
